"""From Selfbot skidding. edited here and there  though  by Liam#3273"""
import discord
...
class Mod:
    ...
    async def format_mod_embed(self, ctx, user, success, method, duration = None, location=None):
        """SEE THE SELFBOT CODE OKAY XD skidding intensefies"""
    ...
    @commands.command()
    async def hban(self, ctx, userid, *, reason=None):
    ...
    async def mute(self, ctx, member:discord.Member, duration, *, reason=None):
        ...
        if ctx.author.guild_permissions.administrator == True or ctx.author.guild_permissions.manage_roles == True:
            ...
            await ctx.send('Muting user!', delete_after=2)
            try:
    ...
    async def unmute(self, ctx, member:discord.Member, *, reason=None):
        ...
        if ctx.author.guild_permissions.administrator == True or ctx.author.guild_permissions.manage_roles == True:
            await ctx.send('Unmuting user!', delete_after=2)
            try:
